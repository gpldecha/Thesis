...
PbD-POMDP approach will not necessarily result in an efficient policy. For the GMM we model
...
convergence \citep{rl_ac_surv_2012}, as the variance of the gradient of the policy's parameters have a smaller 
variance compared with actor methods (policy gradient). This results in a faster learning of the policy. 

Actor-critic \cite[Chap. 6.6]{sutton98a} is an RL approach in which the actor (policy) and critic (value function) 
...
The policy gradient theorem \citep{Sutton00policygradient} states that if the regressor functions of both the
...
This has lead to the application of more sophisticated regression methods such as the increasingly popular Deep Learning methods 
...
%$\hat{x}^{\mathrm{MAP}}_t = \argmax_{x_t} p(x_t|y_{0:t},\dot{x}_{1:t}) \in \mathbb{R}^3$, and the differentiation entropy, 
%$U = H\{p(x_t|y_{0:t},\dot{x}_{1:t})\} \in \mathbb{R}$. All pdfs in our recorded data set $D$ are transformed to 
%a belief space feature vector, $b_t = [\hat{x}^{\mathrm{MAP}}_t,U_t]^{\mathrm{T}}$. 
...
%Both the linear and angular velocity policies are parameterised by a Gaussian Mixture Model (GMM), 
%\begin{equation}
...
\subsection{Fitted Policy Iteration}\label{sec:fpe}
...
%Equation \ref{eq:alpha_eq} - \ref{eq:alpha_expectation}.
%\begin{align}
...
%From both linear and angular velocities a reference position $x^r \in \mathbb{R}^{(3 \times 1)}$ 
%and orientation $R^r \in \mathbb{R}^{(3 \times 3)}$ are computed and used to define a linear and 
%angular error $x_e = x^r - x$, $\psi_e = \mathrm{angleaxis}(R^{\mathrm{T}}R^r)$ by using the  the current position $x$ and orientation $R$.
...
\begin{figure}
 \centering
  ...
  \caption{Distance travelled until the socket's edge is reached. a) Three groups correspond to the initial conditions: Center, Left and Right
   ...
   fixed. The Q-EM method always improves on the GMM. The Q-Greedy policy was learned from data provided by rollouts of a stochastic Greedy policy.}
...
\section{Discussion and Conclusion}\label{ch4:conclusion}
