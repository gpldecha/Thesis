...
During the experiment the position and orientation of the plug is recorded at a rate of a 100 Hz
...
In our setting the belief probability density function,  $p(x_t|y_{0:t},\dot{x}_{0:t})$,  
...
and recursively updated by the application of a \textbf{motion}, $p(x_t|x_{t-1},\dot{x}_t)$ 
and \textbf{measurement}, $p(y_t|x_t)$ model. The motion model updates the position of the probability density function and subsequently increases the uncertainty 
of the position.  The measurement model indicates areas of the state space from which a measurement $\tilde{y}_t$ could have originated. 
In Figure \ref{fig:PMF} (\textit{Right-bottom}) we illustrated the likelihood when an edge is sensed.

Our choice of a PMF, as means to represent the believed location of the plug, 
is motivated by the fact that the sensing likelihoods are non-gaussian and lead to 
multi-modal distributions.  A PMF is able to capture such non-gaussianity whilst remaining fully deterministic 
...
Below we describe the generic experimental taken for the two Groups A \& B of 5 subjects. 
...
\Input{$K$, $\epsilon$, $\{(\B^{[i]}_t,r^{[i]},\B^{[i]}_{t+1})\}_{i=1,\cdots,M}$}
...
\subsection{Fitted policy evaluation and improvement example}

In Figure \ref{fig:fpe_example}, we illustrate an example of how Fitted policy evaluation and improvement works
in a simple 2D example to give an intuition behind the mechanism involved. The \textit{Top-left} subfigure,
illustrates 10 demonstrated trajectories from two teachers (5 each) which end in a goal state. The optimal 
path to reach the goal is a straight from the start to goal state passing in between two obstacles. 
Between the two teachers non of them demonstrated the optimal straight path. If we were to fit a GMM 
$\pi_{\Param}(\U,\X)$ to the demonstrated data and take the policy to be the output of the 
...
behaviour.  No policy trajectories truly replicate the demonstrated behaviour.  In the \textit{Top-right}
subfigure, we apply fitted policy evaluation to the original demonstrated data (discount 
factor $\gamma=0.99$ and reward $r=1$ when goal reached and zero otherwise) and compute the value function.
The \textit{Bottom-right} subfigure illustrates the GMM policy learned via Q-EM detailed above. As 
the advantage function $ A^{\pi}(\X,\U)$ is highest along the start-goal axis, data points from 
both teachers which follow this gradient will be weighted higher than other data points. This 
results in policy much closer to optimality with better quality rollouts than the trajectories generated 
...
\begin{figure}
 ...
 \caption{Fitted policy evaluation \& improvement example. 
  ...
  \textit{Bottom-left:} A GMM is fitted to the teachers original data. The marginal $\pi_{\Param}(\X)$ is 
  plotted in blue and trajectories generated by the policy $\mathbb{E}\{\pi_{\Param}(\U|\B)\}$ in orange.
  \textit{Top-right} \textit{Policy Evaluation:}.  Value function after fitted policy evaluation terminated, the reward function 
  is binary, $r=1$ at the goal and zero otherwise, and a discount factor $\gamma = 0.99$ was used.
  \textit{Bottom-right} \textit{Policy Improvement:} Policy improvement, the GMM was learned with the Q-EM algorithm in which 
  each the weight of each data point is proportional to the advantage function.
 }
...
\begin{figure}
 ...
 \caption{Best and worst trajectories} 
\end{figure}
