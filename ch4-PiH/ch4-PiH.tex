...
The force torque information can be used to rectify this problem [Klas]. A 
...
without some form of compression. One possibility would be E-PCA \cite{E-PCA} which extracts a set of 
...
To give an intuition of fitted policy evaluation and improvement mechanisms, we give a 2D example 
which is illustrate in Figure \ref{fig:fpe_example}. The \textit{Top-left} subfigure,
...
$\pi_{\Param}(\U,\X)$ to the teacher's data and take the policy to be the  
...
by the policy learned via standard EM, see the \textit{Bottom-right} subfigure for an illustrate of 
the policy and trajectories resulting from policy improvement.
...
The same is applied to the demonstrations from the PiH-search task. Figure \ref{fig:Figure1} (\textit{Top})
shows the value function of the most likely state. 
shows the belief space with respect to the value function. As expected, the 
...
close relationship between the best trajectories and the time taken to successfully connect the plug to the socket. 
We can see that the five best trajectories (red) tend to be aligned with the socket (star position in front of socket), 
whilst the five worst are towards the edges of the wall.
...
\begin{figure}
 ...
 \caption{\textit{T:} LWR approximate value function $\hat{V}^{\pi}(\B)$. \textit{Right:} first five best and worst trajectories in terms of the accumulated value.}
 \label{fig:Figure1}
\end{figure}
