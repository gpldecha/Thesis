...
Additionally there may be no single good teacher, however by combining different demonstrated components from 
individual teachers a good search strategy can be extracted.
...
which has been reported and proven \cite{rl_ac_surv_2012} to be more efficient, as the variance of the gradient of the 
policy's parameters have a smaller variance compared with actor methods (policy gradient). This results in a faster learning of 
the policy. 
...
compress the pdf to a belief space vector composed of the maximum a posteriori, $\hat{x}^{\mathrm{MAP}}_t = \argmax_{x_t} p(x_t|y_{0:t},\dot{x}_{1:t}) \in \mathbb{R}^3$, and the differentiation entropy, 
...
angular sensed wrench to angular velocity, $ \pi_{\Param_2} : \phi \mapsto \omega$.
