...
velocity, $\omega \in \mathbb{R}^3$, at each time step.
...
During the experiment the position and orientation of the plug are recorded at a rate of a 100 Hz
using the OptiTrack system which has sub-millimeter tracking accuracy. The force and torque information 
...
For the task in hand, the belief probability density function,  $p(x_t|y_{0:t},\dot{x}_{0:t})$,  
...
To give an intuition behind the mechanism of fitted policy evaluation and improvement, we give a 2D example 
...
The optimal path to reach the goal is to travel in a straight line passing in between two obstacles. 
None of the two teachers demonstrated the optimal straight path. If we fit a GMM 
$\pi_{\Param}(\U,\X)$ to the teacher's data and take the policy to be the output of 
Gaussian Mixture Regression (GMR) $\mathbb{E}\{\pi_{\Param}(\U|\B)\}$ we get the behaviour 
illustrated in the \textit{Bottom-left} subfigure. As different behaviours were demonstrated and encoded 
in the GMM, the trajectory generated from the GMR policy are an average of the original demonstrated 
...
the advantage function $ A^{\pi}(\X,\U)$ is highest along the start-goal axis, data points from 
the teachers which follow this gradient will be weighted higher. This 
results in a policy with better rollouts (closer to the optimal path) than the trajectories generated 
by the policy learned via standard EM. The \textit{Bottom-right} subfigure illustrates of 
...
\begin{figure}
 ...
 \caption{Fitted policy evaluation \& improvement example. 
  ...
  five trajectories which counter the obstacle in front of the goal. The second teacher (red) demonstrates 
  5 trajectories which initially deviate from the goal before passing between the two obstacles. 
  \textit{Bottom-left:} A GMM is fitted to the teachers original data with the EM algorithm. The marginal $\pi_{\Param}(\X)$ is 
  plotted in blue and trajectories generated by the policy $\mathbb{E}\{\pi_{\Param}(\U|\X)\}$ in orange.
  \textit{Top-right} \textit{Policy Evaluation:}.  Value function after fitted policy evaluation terminated, the reward function 
...
The Fitted policy evaluation (FPE) Algorithm \ref{alg:fpe} is applied to the demonstrations of the teachers 
which did the PiH-search task with socket A. In Figure \ref{fig:Figure1}  we illustrate the value function 
of the most likely state after the FPE algorithm converged. As expected, the value function is high closest to 
the socket and low further away and is highest around the axis $z=0$ and $y=0$. When policy improvement via Q-EM is applied
the Gaussian functions of the GMM will to favour these locations. 
...
In Figure \ref{fig:best_worst_traj} we illustrate the best and worst trajectories in terms of accumulated value function.
...
\begin{figure}
 ...
 gain whilst the blue are the worst. The red arrow indicates the heading direction of the teachers when they performed the task. 
 The blue trajectories tend to be to the sides of the wall as it can be more difficult to localise the wall in this starting position. The red trajectories are centred along the y-axis of socket and tend to move in a straight line 
 towards the wall whilst aligning themselves with the axis $z=0$.} 
...
we will contrast the two policies to the what improvements can be achieved with the RL-PbD-POMDP framework.
...
In Figure \ref{fig:policy_vf} we illustrate the policy vector field of the policy learned solely from human demonstrations, named
GMM, and the policy resulting from one round of fitted policy evaluation and improvement, which we call Q-EM. 
...
%
% Make this a new section with title: Robot Implementation. I believe it is the first time you mention that you use a KUKA LWR. 
%  Explain this and state that this a 7 DOF robotic arm. Explain how many DOF you control.
%
...
Cartesian control $u_t = [\dot{x}_t,\omega_t]^{\mathrm{T}}$ to torque commands $\tau \in \mathbb{R}^7$, Equation \ref{eq:torque_control}.
