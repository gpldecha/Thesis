...
for $P(A_t|Y_{0:t},u_{1:t})$. The 1D world in which the agent and object are located has been discretized to 10 states,
...
$P(A_t=s|Y_{0:t},u_{1:t}): \boldsymbol{\theta}_a^{(s)}$, $P(O=s|Y_{0:t},u_{1:t}): \boldsymbol{\theta}_o^{(s)}$. This is in contrast to the Histogram filter in which the marginals are  
...
To get an informative update of the marginals, the random variables $A$ and $O$ must remain 
...
The motion update, Equation \ref{eq:ch5:mlmf_motion_update}, when applied to the joint distribution results in the original marginal $P(A_0)$ being moved along it's axis 
as well as the likelihood functions.  Equation \ref{eq:ch5:mlmf_filter_conditional}, would require an expensive evaluation of Equation \ref{eq:ch5:mlmf_filter_joint} 
at each state $(i,j)$ and will not have to evaluate the full likelihood product. We will demonstrate later that MLMF only evaluates dependent states $(i,j)$ (states which are affected by the likelihood at the current time step).
The MLFM filter is parameterised by the agent and object marginals $P(A_t|u_{1_:t})$, $P(O)$ and the history of likelihood functions, Equation \ref{eq:ch5:mlmf_filter_joint},
...
by the current binary measurement and the offset of the likelihood function along the $i$-axis. This entails that the parameters of Equation \ref{eq:memory}
...
The probability mass which gets evaluated to zero gets re-normalised to the remaining areas of the joint distribution, as it must sum to one.
...
marginal likelihood $P(Y_{0:t}|u_{1:t})$ \textbf{without} having to perform an \textbf{expensive marginalisation} over the entire space of the joint distribution as it was the case for Histogram-SLAM.
...
In order to compute the marginal likelihood (also known as evidence) $P(Y_{0:t}|u_{1:t})$ and the filtered  marginals $P(A_t|Y_{0:t})$,
...
\paragraph{Evidence}\\
The evidence of the measurement $P(Y_{0:t}|u_{1:t})$ is the amount of probability mass which has been removed from the region of the joint 
...
track of how much it should have been normalised. To this end the marginals $P(A_t|u_{1:t})$ and $P(O)$  are never renormalised but are used 
...
There are two different types of marginals used in the MLMF filter. The first set are the marginals: the marginals of the joint distribution, Equation \ref{eq:ch5:mlmf_filter_joint}
...
which is only evaluated in dependent states. The second set of marginals are the filtered belief which updated by evaluated the joint distribution when necessary.
...
dependent $\cap$ and independent $\ominus$ components. 
%\begin{align}
...
distribution which are independent $A \ominus O$ are dimmed and the dependent areas $A \cap O$, where $P(Y_t<1|A_t,O)$, are not dimmed. MLMF only
evaluates the joint distribution where the likelihood changes the joint distribution. For each state $s$ of the marginals $1,\dots,10$ the difference 
...
The illustrated marginals (\textit{Bottom row}) are the filtered marginals. The shape of the joint marginals $P(A_t|u_{1:t})$, $P(O)$
(not shown) will remain unchanged by measurements during the filtering process, they are the parameters of the joint distribution which is used to 
for the update of the filtered marginals $P(A_t|Y_{0:t},u_{1:t})$, $P(O|Y_{0:t})$.
...
subsequently we generalise. As stated previously $M$ stands for the number of filtered random variables including the agent. 
$N$ is the number of discrete states in the world. We contrast the MLMF-SLAM with Histogram-SLAM.
