...
We next demonstrate how the two different parametrisations of the joint distribution of the BN in Figure \ref{fig:bayesian_sse_dag} 
...
The measurement of features $j$ is typically the range and bearing to an observed features, $Y^{(j)}_t = [r,\phi]$,
...
\begin{tabular}{lcl}
 Histogram    & : & $\theta^{(ij)}$ \\
 desired-MLMF & : & $\theta^{(i)}$, $\theta^{(j)},\alpha,\{(\hat{Y}_t,l)\}_{t=0:T}$\\
	      &   & $i,j=1,\dots,N$
\end{tabular}
}
\end{minipage}
...
%\begin{tabular}{lcl}
% Histogram & : & $\theta^{(ij)}$ \\
% MLMF      & : & $\theta^{(i)}$, $\theta^{(j)},\alpha,\{(\hat{Y}_t,l)\}_{t=0:T}$\\%
%	   &   & $i,j=1,\dots,N%$
%\end{tabular}
%\end{minipage}
%};
...
The likelihood function is parametrised by an input, $\hat{Y}_t$ and offset $l$ to the $i$-axis. In Figure \ref{fig:discrete_example} we see that
...
%\begin{equation}
%  P(A_0,O|Y_0,u_0) =   P(A_0) \cdot P(O) \cdot P(Y_0|A_0,O)
%\end{equation}
...
\begin{align} 
 P(A_{t-1},O|Y_{0:t-1},u_{0:t-1}) = &P(A_0) \cdot P(O)  \cdot  P(Y_0|A_0,O) \cdot  \nonumber \\ 
				  &\prod\limits_{t=1}^{t-1} P(A_t|A_{t-1},u_t) \cdot P(Y_t|A_t,O)  \label{eq:memory_function} \\
...
At every time step a new likelihood function is added to Equation \ref{eq:memory_function} and a motion update 
is applied to all the likelihood functions in memory and to the agent's marginal $P(A_0)$.
...
likelihood function) shifts along the agent's $i$-axis by the displacement caused by the forward model and a new line is added at $i=j$, 
...
where the likelihood function smaller than one and the region inside $A \ominus O$ will not be affected, where the likelihood function 
is equal to one.
...
%\begin{FPfigure}
 ...
 $P(Y_t|A_t,O)$ effect on the joint distribution for three different initialisations. 
  The tube area depicts the regions where the measurement function influences the joint distribution, and 
  the values of the random variables in these areas are dependent $A \cap O$. The variables of the joint 
  distribution which are not effected by the likelihood function are independent from one another, $A \ominus O$. 
 ...
 the white space is in the domain but not parametrised and thus not considered. \textbf{a)} The agent and object marginals 
...
%\begin{equation}
%  P(Y_0) = 1 - \overbrace{\sum\limits_{A_0}\sum\limits_{O} P_{\cap}(A_0,O|Y_0) - P(Y_0|A_0,O)\cdot P_{\cap}(A_0,O|Y_0)}^{\alpha_0}
%\end{equation}
...
\begin{align}\label{eq:I}
 \alpha_t &= \sum\limits_{A_t}\sum\limits_{O} \underbrace{P_{\cap}(A_t,O|Y_{0:t-1})}_{P_{\cap}(A_t)\cdot P_{\cap}(O) \cdot P(Y_{0:t-1}|A_t,O) } (1 - P(Y_t|A_t,O)) \\
\end{align}
% \alpha_t &=  \sum\limits_{A_t}\sum\limits_{O} P_{\cap}(A_t)\cdot P_{\cap}(O) \cdot P(Y_{0:t-1}|A_t,O) \Big(1 - P(Y_t|A_t,O)\Big) \nonumber \\
...
\ref{fig:overlap_dependence_independence} (\textit{Middle \& Right}) the sum of the probability mass in the green 
...
\fbox{
\begin{tabular}{lcl}
    functions         &   & parameters \\\hline
  $P(A_t|Y_{0:t})$    & : & $\theta^{(i)}$\\
  $P(O|Y_{0:t})$      & : & $\theta^{(j)}$\\
  $P(Y_t|Y_{0:t-1})$  & : & $\alpha \in \mathbb{R}$\\
  $P(Y_{0:t}|A_t,O)$  & : & $\{(\hat{Y}_t,l)\}_{t=0:T}$ \\
  $P_{\cap}(A_0)$     & : & $\theta_*^{(i)}$ \\
  $P_{\cap}(O)$       & : & $\theta_*^{(j)}$
\end{tabular}
}
...
\begin{algorithm}
\caption{MLMF-SLAM}
\textbf{INPUT}: $P_{\cap}(A_0)$, $P_{\cap}(O)$
 \begin{algorithmic}[1]
   \vspace*{0.25cm}
   \Statex{\textbf{Initialisation}}
   \vspace*{0.25cm}

   \State{$P(A|\emptyset) = P_{\cap}(A_0)$} 
   \State{$P(O|\emptyset) = P_{\cap}(O)$}
   \State{$\alpha         = 0$}
   \State{$P(Y_0|A_0,O)   = \{\}$}
   ...
   \vspace*{0.25cm}
   \State{$P(A_{t-1}|Y_{0:t}) = P(A_{t-1}|Y_{0:t-1}) - \Big(P_{\cap}(A_{t-1}|Y_{0:t-1}) -  P_{\cap}(A_{t-1}|Y_{0:t}) \Big)$} 
   \State{$P(O|Y_{0:t}) = P(O|Y_{0:t-1}) -  \Big(  P_{\cap}(O_t|Y_{0:t-1}) -  P_{\cap}(O_t|Y_{0:t})\Big)$}
   \vspace*{0.25cm}
   \State{$\alpha = \alpha + \sum\limits_{A_t}\sum\limits_{O} P_{\cap}(A_t)\cdot P_{\cap}(O) \cdot P(Y_{0:t-1}|A_t,O) \Big(1 - P(Y_t|A_t,O)\Big) $} 
   \State{$P(Y_{0:t-1}|A_t,O) \leftarrow (\hat{Y}_t,l=0)$}
   \vspace*{0.25cm}
   \Statex{\textbf{Motion Update}}
   \vspace*{0.25cm}
   \State{$P(A_t|Y_{0:t})  = \sum\limits_{A_{t-1}} P(A_t|A_{t-1},u_t) \cdot P(A_t|Y_{0:t}) $}
   \State{$P_{\cap}(A_t)  = \sum\limits_{A_{t-1}} P(A_t|A_{t-1},u_t) \cdot P_{\cap}(A_t) $}
...
\textbf{OUTPUT:} $P(A_t|Y_{0:t})$, $P(O_t|Y_{0:t})$ 
\end{algorithm}
...
\begin{align}
  ...
  P(A_t,O,Y_{0:t},u_{0:t}) &= \sum_{A_{t-1}} P(A_t,A_{t-1},O,Y_t,Y_{0:t-1},u_{t},u_{0:t-1}) \label{eq:br_chain} \\
		           &= \sum_{A_{t-1}} P(Y_t|\Ccancel[red]{Y_{0:t-1}},A_t,\cancel{A_{t-1}},O,\cancel{u_{t}},\cancel{u_{0:t-1}})\cdot \nonumber \\
		           &\;\;\;\;\;\;\;\;\;\;\;\;       P(A_t|A_{t-1},O,u_{t},\cancel{Y_{0:t-1}},\cancel{u_{0:t-1}}) \cdot \nonumber  \\
		           & \;\;\;\;\;\;\;\;\;\;\;\;      P( A_{t-1},O,\Ccancel[red]{u_t},Y_{0:t-1},u_{0:t-1}) \label{eq:cancellations} \\
		           &= \sum_{A_{t-1}} P(Y_t|A_{t},O) \cdot P(A_t|A_{t-1},u_{t}) \cdot \nonumber \\
		           &\;\;\;\;\;\;\;\;\;\;\;\;  P(A_{t-1},O,Y_{0:t-1},u_{0:t-1}) \\
		           &= P(Y_t|A_{t},O) \cdot \underbrace{\sum_{A_{t-1}} P(A_t|A_{t-1},u_t) \cdot P(A_{t-1},O,Y_{0:t-1},u_{0:t-1})}_{P(A_t,O,Y_{0:t-1},u_{0:t})} \\
		          &= P(Y_t|A_{t},O) \cdot P(A_t,O,Y_{0:t-1},u_{0:t})
\end{align}
