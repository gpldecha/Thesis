...
\begin{tabular}{lcl}
 Histogram    & : & $\theta^{(ij)}$ \\
 desired-MLMF & : & $\theta^{(i)}$, $\theta^{(j)},\alpha,\{(Y,l)_p\}_{p=0:t}$\\
	      &   & $i,j=1,\dots,N$
\end{tabular}
}
\end{minipage}
...
%\begin{algorithm}
%\caption{MLMF-SLAM}
%\textbf{INPUT}: $P(A_0)$, $P(O)$, memory = $\{(Y,l)_t\}_{t=0:t}$
% \begin{algorithmic}[1]
%   \vspace*{0.25cm}
%   \Statex{\textbf{Initialisation}}
%   \vspace*{0.25cm}

%   \State{$P(A|\emptyset) = P(A_0)$} 
%   \State{$P(O|\emptyset) = P(O)$}
%   \State{$\alpha         = 0$}
%   \State{memory $= \{\}$}
...
%   \vspace*{0.25cm}
%   \State{$\alpha = \alpha +  \sum\limits_{A_t}\sum\limits_{O} P_{\cap}(A_t,O|Y_{0:t-1}) (1 - P(Y_t|A_t,O))  $} 
%   \State{memory $\leftarrow (Y,l=0)_t$}
%   \vspace*{0.25cm}
%   \Statex{\textbf{Motion Update}}
%   \vspace*{0.25cm}
%   \State{$P(A_t|Y_{0:t})  = \sum\limits_{A_{t-1}} P(A_t|A_{t-1},u_t) \cdot P(A_t|Y_{0:t}) $}
%   \State{$P(A_0)  = \sum\limits_{A_{t-1}} P(A_t|A_{t-1},u_t) \cdot P(A_0) $}
%   \State{memory $=$ $\{(Y,l + u_t)\}$}
%   
