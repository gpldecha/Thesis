...
of dependent states $|A \cap O|$. In Figure \ref{fig:ch5:marginal_update}, this corresponds to the states in the joint distribution for 
which $i = j$ and there are $N$ of them for a joint distribution with $N^2$ states. In Figure \ref{fig:3bel_lik_profile} \textit{left}
...
giving a total of $2\,N^2$ dependent states. Let's see why it is $N^2$ for each likelihood, the overall likelihood is given by 
...
In general, for $M$ random variables the computational cost is $(M-1) N^{M-1}$ which gives $\BigO(N^{M-1})$ as opposed to $\BigO(N^M)$. 
