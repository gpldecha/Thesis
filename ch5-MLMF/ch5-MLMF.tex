...
likelihood functions, $P(Y_{0:t}|A_t,O,u_{1:t};\mathcolor{dark-green}{\BPsi})$ Equation \ref{eq:memory}, which is all the 
likelihood functions since $t=0$ until $t$ and we will refer to it as the \textbf{memory likelihood function}: 
...
The memory likelihood function's parameters $\BPsi = \{(Y_p,l_p)\}_{p=0:t}$ consist of a set of measurements $Y_{0:t}$ and offsets $l_{0:t}$
depicted in greed. The measurements $Y_t \in \{0,1\}$ are always binary, whilst the offsets $l_i$, actions $u_t$, 
agent $A_t$ and object $O$ variables' size are equal to the dimension of the state space. The subscribe $i$ 
of an offset $l_i$ indicates which likelihood function it belongs to. The offset of a likelihood function is given by the 
summation of all the applied actions since $t=0$ until $t=i$, Equation \ref{eq:ch5:offset}, which can be computed recursively.
...
In Algorithm \ref{alg:memory-motion}, we detail how an action $u_t$ and measurement $Y_t$, result in the update from $\Psi_{0:t-t}$ to $\Psi_{0:t}$, 
the parameters of the likelihood functions; this is an implementation of Equations \ref{eq:ch5:mlmf_motion_update}-\ref{eq:ch5:mlmf_measurement_update} 
for the memory likelihood function.
...
Since initialisation of the joint distribution at $t=0$ until $t=3$ the object's marginal $P(O;\mathcolor{red}{\ThOs})$ remains unchanged and the agent's 
marginal $P(A_2|u_{1:2};\mathcolor{blue}{\ThAs})$ is updated by the two actions according to the motion update, see Figure \ref{fig:maringal_joint_example_v2} \textit{Top-right}.
...
\begin{figure}
 ...
 bottom of the figure which is $P(Y_{0:3}|A_2,O,u_{1:2})$. \textit{Right column:} the top figure illustrates the original marginal of the 
...
twice which results in a $\mathcolor{dark-green}{l_0}=2$ parameter for the first likelihood function. We highlighted, in the Figure, the likelihood in light-green 
to indicate that it was the first added to the memory term and for easy comparison with Figure \ref{fig:discrete_example} on page \pageref{fig:discrete_example}. As for the second 
...
function $P(\mathcolor{dark-green}{Y_3}|A_2,O;\mathcolor{dark-green}{l_3=0})$ has not yet been updated by the next action $u_3$. The parameters of likelihood product, Equation \ref{eq:memory}, are: $\Psi_{0:2} = \{(0,2)_{p=0},(0,1)_{p=1},(0,0)_{p=2}\}$ and the evaluation of memory 
...
There are two different types of marginals used in the MLMF filter. The first set are the \textbf{joint marginals} of the joint distribution, Equation \ref{eq:ch5:mlmf_filter_joint}
...
\begin{align}
 &P(A_t|Y_{0:\mathbf{t}};\mathcolor{blue}{\ThA}) = P(A_t|Y_{0:\mathbf{t-1}};\mathcolor{blue}{\ThA}) - \hspace*{5cm} \nonumber \\
 &\hspace*{3cm}  \Big(P_{\cap}(A_t|Y_{0:t-1}) -  \mathbf{P_{\cap}(A_t|Y_{0:t}})  \Big)  \label{eq:marignal_mrf} 
\end{align}
\begin{align}
 &\mathbf{P_{\cap}(A_t|Y_{0:t}};\mathcolor{dark-blue}{\Alpha},\mathcolor{dark-green}{\bar{\Psi}_{0:t}},\mathcolor{blue}{\ThAs}, \mathcolor{red}{\ThOs})  =  \nonumber \\[0.5cm]
 &\frac{ \sum\limits_{O} P(Y_t|A_{t},O) P_{\cap}(O;\mathcolor{red}{\ThOs}) P_{\cap}(A_t|u_{1:t};\mathcolor{blue}{\ThAs})  P(Y_{0:t}|A_t,O,u_{1:t};\mathcolor{dark-green}{\bar{\Psi}_{0:t}})}{P(Y_{0:t}|u_{1:t};\mathcolor{dark-blue}{\Alpha})} \label{eq:marignal_mrf_2} 
\end{align}

%\begin{align}
%  &P_{\cap}(A_t,O,Y_{0:t-1}|u_{1:t}; \mathcolor{dark-green}{\bar{\Psi}_{0:t}},\mathcolor{blue}{\ThAs}, \mathcolor{red}{\ThOs}) = \nonumber \\
%  &P_{\cap}(O;\mathcolor{red}{\ThOs}) P_{\cap}(A_t;\mathcolor{blue}{\ThAs})  P(Y_{0:t}|A_t,O,u_{1:t};\mathcolor{dark-green}{\bar{\Psi}_{0:t}})
%\end{align}
...
\end{figure}
In Equation \ref{eq:marignal_mrf_2} we explicitly show the relation between the parameters $\bar{\Psi}_{0:t}$, $\ThAs$, $\ThOs$, $\Alpha$ and the joint
distribution at the dependent states. Equation \ref{eq:marignal_mrf} is recursive $P(A_t,|Y_{0:\mathbf{t}})$ is computed in terms of $P(A_t|Y_{0:\mathbf{t-1}})$. 
