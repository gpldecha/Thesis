...
\begin{align}
 ...
 & P(A_t,O|Y_{0:t},u_{1:t}) = \frac{P(Y_t|A_t,O) P(A_t,O|Y_{0:t-1},u_{t}) }{P(Y_t|Y_{0:t-1})} \label{eq:ch5:disc_measurement} 
\end{align}
\end{minipage}
};
...
\For{$l_{t-1} \in \Psi_{t-1}$}{$l_t = l_{t-1} + u_t$}
\textbf{measurement update}\\
$\Psi_t \gets \{\Psi_{t-1}, (Y_t,l=0)\}$ 

\caption{Likelihood functions update}
...
\BlankLine
\textbf{motion update}\\
$P(A_t|u_{1:t})  = \sum\limits_{A_{t-1}} P(A_t|A_{t-1},u_t)  P(A_{t-1}|u_{1:t-1})$\\
$P(A_t|Y_{0:t-1},u_{1:t})  = \sum\limits_{A_{t-1}} P(A_t|A_{t-1},u_t)  P(A_{t-1}|Y_{0:t-1},u_{1:t-1})$\\
...
the worst case scenario the space complexity of the memory will be $N$, giving a total of $M (2 N) + N$. 
