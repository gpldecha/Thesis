...
of the individual Gaussian functions, $g(\cdot)$. The objective of a Reinforcement Learning (RL) is 
to have the policy maximise a reward function, $J(\Param)$ Equation \ref{eq:ch4:app:J}, defined over the policies' parameters:
...
Most policy search methods have at their core the above gradient estimate method. The drawback of policy gradient methods is that a learning 
...
\begin{equation}
  \Param^{\mathrm{new}} = \argmax\limits_{\Param} \overbrace{\sum_{i \in \mathbb{T}} \pi_{\Param}(\Ui,\Bi)\, R(\Ui,\Bi)}^{J(\Param) } \label{eq:ch4:argmax}
\end{equation}

$\mathbb{T}$ is the dataset of all episodes. The above equation can be solved through through the same approach taken in EM 
which consists of maximising the the logarithmic lower bound of the MC approximation of the cost function $J(\Param)$. 
...
\begin{align}
  \log( J(\Param) )  &= \log \sum_{i \in \mathbb{T}} \frac{\pi_{\Param}(\Ui,\Bi)}{\pi_{\Param'}(\Ui,\Bi)} \, \pi_{\Param'}(\Ui,\Bi) \, R(\Ui,\Bi) \nonumber \\
		     & \geq \underbrace{\sum_{i \in \mathbb{T}}\log\Bigg( \frac{\pi_{\Param}(\Ui,\Bi)}{\pi_{\Param'}(\Ui,\Bi)}\Bigg) \, \pi_{\Param'}(\Ui,\Bi) \, R(\Ui,\Bi)}_{\mathcal{Q}(\Param,\Param')} \label{eq:lower_bound}
\end{align}
The inequality is achieved through using Jensen's inequality. We maximise the lower bound, 
$\nabla_{\Param}\mathcal{Q}(\Param,\Param') = 0$, by setting its derivative to zero. The parameter
$\Param'$ are the old parameters used by the policy to generate the episodes through MC, which is the E-step.
...
\section{Q-EM for GMM derivation}\label{app:grad}
