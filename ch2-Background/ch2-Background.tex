...
is located are humid, misty or dark, for instance, it can make it difficult for the robot to ascertain its position and 
to plan how to achieve a given objective.

Given these two types of uncertainty, the question is on how to represent it. The predominant approach 
...
This quantification of action and observation uncertainty in terms of a probability distribution over the state must
be utilised by the agent to plan actions towards accomplishing its goal. To take a decision the agent must assign a utility 
...
When referring outright to decision theory with no extensions, we are usually referring to a one shot
...
of  a sequence of states $u(x_{0:T})$ weighted by its probability. The graphical representation (Figure \ref{fig:mdp} (a)) allows to read 
off directly the probability of a sequence of states and actions,
...
markov assumption and that current rewards do not dependent future rewards, the sums can be re-arranged and 
a recursive patter emerges which can be exploited which results in Equation \ref{eq:expansion}
...
\end{equation}
For the terminal state $V_T(x_T) = u(x_T)$. The bellman equation is a means of solving a sequential decision problem 
...
and value function are obtained through maximising the expected utility. The re-arrangement of the sums, known as
...
be Gaussian (usually the case), non-Gaussian, state dependent or not. The uncertainty of the latent state, $x_t$, is quantified by a probability distribution, $p(x)$. This probability distribution represents all the hypothetical positions in the world in which the agent can be. In Figure \ref{fig:belief_update_example} \textbf{(a)} an agent is located in a square yard 
...
his sensors which provide range, $r$, and bearing, $\phi$, information to the wall, see Figure \ref{fig:belief_update_example} \textbf{(b)}.
The agent uses the model of his sensor, known a priori, to deduce all possible locations in the world where the current 
...
as in the MDP, introduced in the previous section, but the state space $X$ is latent and 
...
Because the state space is partially observable the expected utility has to be computed for each possible history of states, actions and observations.
...
\end{equation}
Using this equation in this form is problematic, we are summing over the space of beliefs and the 
transition function is a probability distribution over beliefs. The key to overcome this problem is to 
realise that if we know what the current measurement and applied action are there is only one valid possible belief, $b_{t+1}$, and
...
\end{equation}
After the substitution and re-arrangement of the sums we get a  over all future value 
functions weighted by their probability, see Equation \ref{eq:max_component}. 
Since the observation is known (because the outer integral is over $y_t$), the integral over the beliefs vanishes since there is only one possible future belief which is given by the 
...
It can be solved with value iteration but each backup operation (application of the bellman equation) 
results in an exponential growth in the number of parameters to represent the value function, which is computationally intractable.
...
updates is Piece Wise Linear and Convex (PWLC). A good text on the implementation of exact value iteration for POMDPs can be found \cite[Chap. 15]{Thrun_2005} and here \cite{Kaelbling_1998}.
...
\begin{itemize}
 ...
 choice will greatly impact the the computational cost of VI.
 \item \textbf{curse of history}: The space and computationally complexity, in the worst case, is exponential with respect to the planning 
...
techniques have emerged with some being very successful. In the next section, we survey the literature the developments of approximate POMDP
...
\include{./ch2-Background/scientific-interactions}
...
the distance between the successive roll-out trajectory distributions. Previous policy gradient methods did not have such guarantees,
...
PI2 stuff just mention it and wrap it up: \cite{dmp_seq_2012} \cite{dmp_iros_2011} % Ok can do grasping use PI2 
...
\cite{Uncer_reduction_heuristic_2015} % uncertainty reduction via heuristic search

			% Active Policy Learning for robot planning and exploration under uncertainty coste function optimsation
%\cite{plan_cont_bel_space_2015}		% Belief space approach to autonomous navigation
