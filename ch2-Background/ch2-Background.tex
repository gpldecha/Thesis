...
\subsection{Approximate Value function}
...
These methods are well suited to address problems which are easily expressed in a discrete state space. All examples problems are simulation and 
no physical interactive problems are considers. One of the main reasons for that all variables are discrete. Besides the belief set generation problem,
interest has also been poised on porting the PBVI to a continuous state space.
...
A example of a continuous action space PBVI method is Ca-Perseus \cite{Spaan05icra}, in which the authors replace the 
maximisation over the action by instead sampling them from a parametric continuous representation. In \cite{PBVI_C_2006}
the state space, transition and observation model are represented by Gaussian Mixtures and the authors consider a particle 
set or Gaussian mixture representation of the belief. The authors show that a continuous representation of the state space
preserves the PWLC property of the value function. They extend there method to continuous action and observations through 
a sampling instead of discretising. Results are shown in 1D continuous corridor setting.
...
\cite{MC-POMDP}		  % first real usage of continuous action, observation, real-time dynamic programming
\cite{solving_continous_pomdps_2013} % Efficient state space representation

% Historical implementation of PBVI
% A POMDP Approach to Robot Motion Planning under Uncertainty
% Popular approach
   
\cite{Kurniawati2009MotionPU}
\cite{Ross08onlineplanning} % Online planning algorithms
