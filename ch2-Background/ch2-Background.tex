...
\node [white_box] (box){%
     ...
     The Bayesian filter turns a prior probability distribution over the state space, $p(x_{t-1}|y_{0:t-1},a_{0:t-1})$,
  ...
  \textbf{Motion update}
	\begin{equation}\label{eq:motion_update}
	  p(x_t|y_{0:t-1},a_{0:t}) = \int p(x_t|x_{t-1},a_t)\, p(x_{t-1}|y_{0:t-1},a_{0:t-1})\;dx_{t-1}
	\end{equation}
...
\begin{itemize}
 \item Value Iteration (VI) \ref{lit:VI}
 \item Policy Search \ref{lit:policy_search}
 \item Planning \ref{lit:Planning}
 \item Heuristics \ref{lit:heuristics}
\end{itemize}
