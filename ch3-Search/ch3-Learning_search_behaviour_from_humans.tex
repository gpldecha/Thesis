...
To achieve this transfer of behaviour from a human teacher to the robotic apprentice we apply the framework outlined in Chapter 2, 
...
%\begin{itemize}
% \item Do humans exhibit particular search strategies and if so is it feasible to learn them ?
% \item How well does a statistical controller learned from human demonstrations perform with
% respect to approaches which do not take into account the uncertainty directly ?
%\end{itemize}

%Before each demonstration the human volunteer is disoriented. His transitional position is varied with 
%respect to the table although his heading remains the same (facing the table) leaving no uncertainty 
%on his orientation. The disorientation of the human subject is to ensure that his believed location is uniform. 
%At the first time step, the human's state of mind can be considered observable. All proceeding 
%beliefs can then be recursively estimated from the initial belief. 


%Learning from demonstrations (LfD) has long been a successful method for transferring
%knowledge from human teachers to robots. A common problem which arises in robotic
%navigation is how to take into account appropriately state uncertainty. To consider whether
%humans are suitable teachers for showing robots how to handle this uncertainty we compare
%human policies with that of an ideal agent, modelled by a POMDP
...
Spatial navigation,\cite{Wang_2007}, \cite{what_det_our_nav_ability_2010}, focuses on the role sensory perception 
...
%In this work we investigate human's spatial navigation abilities in accomplishing a haptic search task for specific set of objects 
%which are distributed in a home environment. The environment is set in the dark to make it a strictly haptic 
%exploration task leaving vestibular, proprioceptive and somesthetic senses. We also implicitly exclude auditory and olfactory information 
%since the search task in question contains no cues sensitive to these senses and as a result are of no help in accomplishing the 
%task. 
...
has to infer the humans mental belief, such that he knows 
...
The probability distribution over the state $p(x_{t}|z_{0:t})$ is represented by a set of weighted particles which represent hypothetical
...
% The edge and corner feaures are employed in trajectory classification
% (risk-averse and risk-prone). The authors are suggested to discuss whether this
% choice of features is suitable for other learning tasks. Is the choice of features for
% trajectory classification task dependent?
...
\subsection{Time efficiency \& Uncertainty}
...
In experiment \#2, both Greedy and Coastal policies perform equally well and will usually perform faster than the GMM model if the true and believed locations of the 
...
The rows in Table \ref{tab:mean-var-distance} for the Greedy (\#2) and Coastal navigate (\#4) are an order of magnitude faster than the GMM method. 
